...
The three-way comparer uses a separate NeoForm-only named source result as the base, automatically writes non-conflicting NeoForge-only and Paper/Victus-only changes to `build/neoforge/merge/overlay`, and writes `build/neoforge/merge/conflicts.json`. Any class changed differently by both sides is unresolved unless an owned replacement exists at the same relative path below `hybrid-neoforge/bridge-patches/`. Full replacements now cover the first FML/Paper boot, pack/network, and registry batches, including `Main`, `MinecraftServer`, `DedicatedServer`, `ReloadableServerResources`, `Connection`, `PacketEncoder`, `ServerConnectionListener`, `PackRepository`, `ServerPacksSource`, and `MappedRegistry`. These retain Paper CLI/plugin/tick/network/resource behavior while composing NeoForge mod loading, lifecycle, data-map/datapack, reload-listener, connection-filter, codec, server-event-loop, dual-stack, and root/child pack hooks; nearby `Victus bridge` comments identify non-obvious joins. `Main.java.patch` remains provenance for the required FML `main(String[])` to Paper `OptionSet` entrypoint bridge. Complete source jars preserve deletions; generated Paper source directories are overlays and may declare explicit removals in `.hybrid-deletions`. The established semantic three-way comparison now contains **320 unresolved classes**, down from 326 before the current bridge batch, with **15 resolved by bridge**. The explicit `MappedRegistry` replacement retains Paper's registry builder prefill API, fluid cache hook, intrusive-holder cleanup, pre-sized maps, and freeze validation while composing NeoForge aliases, explicit IDs, registration/bake/clear callbacks, repeatable unfreeze/freeze, snapshot remapping, and pending-tag contents. Registry writes remain externally serialized by Paper's `RegistryLoadTask` lock and its concurrent holder getter; this class deliberately preserves the upstream single-writer lifecycle rather than implying general concurrent mutation safety. Distribution assembly remains fail-closed while `unresolved` is non-empty.
