...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    public void channelActive(final ChannelHandlerContext ctx) throws Exception {
        ...
        // Victus bridge (NeoForge): bind loader connection state after Paper has captured the channel/address.
    ...
    public void setupOutboundProtocol(final ProtocolInfo<?> protocol) {
        ...
        if (bundlerInfo != null) {
            ...
            configMessage = configMessage.andThen(ctx -> {
                ctx.pipeline().addAfter(HandlerNames.ENCODER, HandlerNames.UNBUNDLER, newUnbundler);
                // Victus bridge (NeoForge): filters must remain between Paper's named encoder and
                // the newly installed unbundler whenever configuration changes protocol.
    ...
    public static ChannelFuture connect(final InetSocketAddress address, final EventLoopGroupHolder eventLoopGroupHolder, final Connection connection) {
        // Victus bridge (NeoForge): validate dual-stack routing before Paper/Netty initiates connect.
    ...
    // Victus bridge: NeoForge filters and handshake extensions need access to Paper's live channel
    // and negotiated inbound protocol; expose the same accessors as the patched loader source.
    public Channel channel() {
        return this.channel;
    }
