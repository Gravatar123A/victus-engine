 ...
 * Base: Minecraft 26.2 / Paper patched ServerCommonPacketListenerImpl.
 * NeoForge side: 26.2.0.57 custom payload dispatch, channel state, packet validation,
 * connection-type propagation, and ICommonPacketListener accessors.
 * Paper remains authoritative for packet threading, keepalive abuse checks, plugin
 * messaging, kick events, asynchronous disconnects, and listener handoff state.
 */
...
public abstract class ServerCommonPacketListenerImpl implements ServerCommonPacketListener {
    ...
    /** Victus bridge (NeoForge): retained across configuration/game listener handoffs. */
    ...
    public ServerCommonPacketListenerImpl(final MinecraftServer server, final Connection connection, final CommonListenerCookie cookie) {
        ...
        // Paper end
        // Victus bridge (NeoForge): CommonListenerCookie is bridged with both Paper state and this type.
    ...
    public void handleCustomPayload(final ServerboundCustomPayloadPacket packet) {
        // Victus bridge (NeoForge): decoded NeoForge/common payloads stay on NeoForge's network-thread
        // path. Paper's plugin messaging path below retains its main-thread handoff and anti-abuse failure.
        if (packet.payload() instanceof net.neoforged.neoforge.network.payload.MinecraftRegisterPayload payload) {
            net.neoforged.neoforge.network.registration.NetworkRegistry.onMinecraftRegister(this.getConnection(), payload.newChannels());
            return;
        }
        if (packet.payload() instanceof net.neoforged.neoforge.network.payload.MinecraftUnregisterPayload payload) {
            net.neoforged.neoforge.network.registration.NetworkRegistry.onMinecraftUnregister(this.getConnection(), payload.forgottenChannels());
            return;
        }
        if (packet.payload() instanceof net.neoforged.neoforge.network.payload.CommonVersionPayload payload) {
            net.neoforged.neoforge.network.registration.NetworkRegistry.checkCommonVersion(this, payload);
            return;
        }
        if (packet.payload() instanceof net.neoforged.neoforge.network.payload.CommonRegisterPayload payload) {
            net.neoforged.neoforge.network.registration.NetworkRegistry.onCommonRegister(this, payload);
            return;
        }
        ...
        try {
            ...
            if (registerChannel || CUSTOM_UNREGISTER.equals(identifier)) {
                // Strings separated by zeros instead of length prefixes. readChannelIdentifier keeps
                // NeoForge's ad-hoc channel view and Paper's plugin messenger view in sync.
                int startIndex = 0;
    ...
    private void readChannelIdentifier(final byte[] data, final int from, final int to, final boolean register) {
        ...
        final net.minecraft.resources.Identifier channelId = net.minecraft.resources.Identifier.tryParse(channel);
        if (channelId != null) {
            if (register) {
                net.neoforged.neoforge.network.registration.NetworkRegistry.onMinecraftRegister(this.getConnection(), java.util.Set.of(channelId));
            } else {
                net.neoforged.neoforge.network.registration.NetworkRegistry.onMinecraftUnregister(this.getConnection(), java.util.Set.of(channelId));
            }
        }
        if (register) {
    ...
    public void send(final Packet<?> packet, final @Nullable ChannelFutureListener listener) {
        // Victus bridge (NeoForge): refuse unnegotiated mod payloads before Paper queues the packet.
    ...
    protected CommonListenerCookie createCookie(final ClientInformation clientInformation) {
        return this.createCookie(clientInformation, this.connectionType);
    }

    protected CommonListenerCookie createCookie(
        final ClientInformation clientInformation,
        final net.neoforged.neoforge.network.connection.ConnectionType connectionType
    ) {
    ...
    // Victus bridge (NeoForge): ICommonPacketListener contract used by codecs, handlers, and tasks.
    @Override
