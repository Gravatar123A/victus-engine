...
public class ServerConnectionListener {
    ...
    public void startTcpServerListener(final SocketAddress address) throws IOException {
        // Paper end - Unix domain socket support
        // Victus bridge: NeoForge configures Netty's IP family only for TCP addresses; Paper may bind Unix sockets here too.
        if (address instanceof java.net.InetSocketAddress inetAddress) {
            net.neoforged.neoforge.network.DualStackUtils.checkIPv6(inetAddress.getAddress());
        }
                        ...
                        .childHandler(
                            new ChannelInitializer<Channel>() {
                                ...
                                protected void initChannel(final Channel channel) {
                                    ...
                                    ChannelPipeline pipeline = channel.pipeline().addLast(HandlerNames.TIMEOUT, new ReadTimeoutHandler(READ_TIMEOUT));
    ...
    public void acceptChannel(final Channel channel, final UUID profileId) {
                ...
                new ChannelInitializer<Channel>() {
                    ...
                    protected void initChannel(final Channel ch) {
                        ...
                        Connection connection = rateLimitPacketsPerSecond > 0
                            ? new RateKickingConnection(rateLimitPacketsPerSecond)
                            : new Connection(PacketFlow.SERVERBOUND);
                        ChannelPipeline pipeline = ch.pipeline().addLast(HandlerNames.TIMEOUT, new ReadTimeoutHandler(READ_TIMEOUT));
                        Connection.configureSerialization(pipeline, PacketFlow.SERVERBOUND, false, null);
                        connection.configurePacketHandler(pipeline);
        ...
        EventLoopGroupHolder.local().serverEventLoopGroup().register(channel).syncUninterruptibly();
    }
