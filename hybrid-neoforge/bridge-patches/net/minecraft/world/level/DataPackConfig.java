...
public class DataPackConfig {
    ...
    public DataPackConfig(final List<String> enabled, final List<String> disabled) {
        // Victus bridge (NeoForge): loader-discovered packs are appended during Paper repository
        // configuration, so the enabled side must remain mutable while disabled stays immutable.
    ...
    public void addModPacks(final List<String> modPacks) {
        modPacks.stream().filter(pack -> !this.enabled.contains(pack)).forEach(this.enabled::add);
    }
}
